...
async function getPublicUploadsAllowed() {
  const { data, error } = await getClient()
    .from("app_settings")
    .select("value")
    .eq("key", "public_uploads")
    .maybeSingle();

  if (error) {
    // Table may not exist yet
    console.warn("Could not read public_uploads setting:", error.message);
    return false;
  }
...
async function setPublicUploadsAllowed(allowed) {
  await refreshAdminState();
  if (!isAdmin()) {
    throw new Error("Admin login required to change upload permissions.");
  }
...
async function canAddDocuments() {
  await refreshAdminState();
  if (isAdmin()) return true;
  return getPublicUploadsAllowed();
}
...
async function saveFile(library, year, file, semester) {
  if (!(await canAddDocuments())) {
    throw new Error("Uploading is disabled. Ask an admin to allow public uploads, or log in as admin.");
  }
...
window.getPublicUploadsAllowed = getPublicUploadsAllowed;
window.setPublicUploadsAllowed = setPublicUploadsAllowed;
window.canAddDocuments = canAddDocuments;
